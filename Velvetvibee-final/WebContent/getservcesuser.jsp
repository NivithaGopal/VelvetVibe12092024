...
%>
    ...
    id="favButton<%= velvetService.getServicevv_id() %>" 
    data-user-id="<%= user %>" 
    data-servicevv-id="<%= velvetService.getServicevv_id() %>">
    Add to Favourite
</button>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        var bookNowButtons = document.querySelectorAll('[id^="bookNowButton"]');

        bookNowButtons.forEach(function(button) {
            button.addEventListener('click', function() {
                var serviceId = this.id.replace('bookNowButton', '');
                var serviceName = this.getAttribute('data-service-name'); // Using data attributes to store service name

                if (serviceId) {
                    window.location.href = 'scheduleAppointment.jsp?serviceId=' + encodeURIComponent(serviceId) + '&serviceName=' + encodeURIComponent(serviceName);
                }
            });
        });
    });
...
<script>
document.addEventListener('DOMContentLoaded', function() {
    var favButtons = document.querySelectorAll('[id^="favButton"]');

    favButtons.forEach(function(button) {
        button.addEventListener('click', function() {
            var userId = this.getAttribute('data-user-id');
            var servicevvId = this.getAttribute('data-servicevv-id');

            if (userId && servicevvId) {
                window.location.href = 'addToFav.jsp?userId=' + encodeURIComponent(userId) + '&servicevvId=' + encodeURIComponent(servicevvId);
            }
        });
    });
});
